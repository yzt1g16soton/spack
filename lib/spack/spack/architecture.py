...
class NoPlatformError(serr.SpackError):
    def __init__(self):
        super(NoPlatformError, self).__init__("Could not determine a platform for this machine.")
...
class Platform(object):
    ...
    def add_target(self, name, target):
        ...
        if name in Platform.reserved_targets:
            raise ValueError(
                "%s is a spack reserved alias " "and cannot be the name of a target" % name
            )
...
class Arch(object):
    ...
    def __nonzero__(self):
        return self.platform is not None or self.os is not None or self.target is not None
...
def arch_for_spec(arch_spec):
    ...
    if not (arch_plat.operating_system(arch_spec.os) and arch_plat.target(arch_spec.target)):
        raise ValueError(
