...
class ViewDescriptor(object):
    def __init__(
        self, base_path, root, projections={}, select=[], exclude=[], link=default_view_link
    ):
        self.base = base_path
    ...
    def view(self, new=None):
        ...
        return YamlFilesystemView(
            root, spack.store.layout, ignore_conflicts=True, projections=self.projections
        )
...
class Environment(object):
    ...
    def _get_overwrite_specs(self):
        ...
        for dag_hash in self.concretized_order:
            ...
            ret.extend(
                [d.dag_hash() for d in spec.traverse(root=True) if self._spec_needs_overwrite(d)]
            )
    ...
    def install_specs(self, specs=None, args=None, **install_args):
        ...
        install_args["overwrite"] = install_args.get("overwrite", []) + self._get_overwrite_specs()
    ...
    def _read_lockfile_dict(self, d):
        ...
        for dag_hash, node_dict in json_specs_by_hash.items():
            for dep_name, dep_hash, deptypes in Spec.dependencies_from_node_dict(node_dict):
                specs_by_hash[dag_hash]._add_dependency(specs_by_hash[dep_hash], deptypes)
