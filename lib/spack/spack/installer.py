...
class PackageInstaller(object):
    ...
    def _setup_install_dir(self, pkg):
        ...
        if not os.path.exists(pkg.spec.prefix):
            tty.verbose("Creating the installation directory {0}".format(pkg.spec.prefix))
    ...
    def _update_failed(self, task, mark=False, exc=None):
        ...
        for dep_id in task.dependents:
            if dep_id in self.build_tasks:
                ...
                tty.verbose(
                    "No build task for {0} to skip since {1} failed".format(dep_id, pkg_id)
                )
            ...
            with fs.working_dir(pkg.stage.source_path):
                ...
                for i, (phase_name, phase_attr) in enumerate(
                    ...
                    try:
                        ...
                        with log_output(log_file, echo, True, env=unmodified_env) as logger:

                            with logger.force_echo():
                                inner_debug_level = tty.debug_level()
                                tty.set_debug(debug_level)
                                tty.msg("{0} Executing phase: '{1}'".format(pre, phase_name))
