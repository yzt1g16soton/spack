...
def setup_parser(subparser):
    ...
    create.add_argument(
        "-y", "--spec-yaml", default=None, help="Create buildcache entry for spec from yaml file"
    )
    create.add_argument(
        "--only",
    ...
    dlkeys.add_argument("-t", "--trust", action="store_true", help="trust all downloaded keys")
    dlkeys.add_argument("-f", "--force", action="store_true", help="force new download of keys")
    ...
    saveyaml.add_argument("--root-spec", default=None, help="Root spec of dependent spec")
    ...
    saveyaml.add_argument(
        "-y", "--yaml-dir", default=None, help="Path to directory where spec yamls should be saved"
    )
    ...
    update_index = subparsers.add_parser("update-index", help=buildcache_update_index.__doc__)
...
def match_downloaded_specs(pkgs, allow_multiple_matches=False, force=False, other_arch=False):
    """Returns a list of specs matching the not necessarily
       concretized specs given from cli
...
def _createtarball(
    ...
    for match in matches:
        ...
        if match.external or match.virtual:
            ...
            for d, node in match.traverse(order="post", depth=True, deptype=("link", "run")):
                ...
                if node.external or node.virtual:
                    tty.debug("skipping external or virtual dependency %s" % node.format())
                elif lookup is None:
...
def createtarball(args):
    ...
    if args.directory:
        ...
        if result.name == "<unnamed>":
            raise ValueError('no configured mirror named "{name}"'.format(name=output_location))
...
def installtarball(args):
    ...
    if not args.specs:
        tty.die("build cache file installation requires" + " at least one package spec argument")
    pkgs = set(args.specs)
...
def install_tarball(spec, args):
    ...
    if s.concrete and package.installed and not args.force:
        ...
        if tarball:
            ...
            bindist.extract_tarball(spec, tarball, args.allow_root, args.unsigned, args.force)
...
def download_buildcache_files(concrete_spec, local_dest, require_cdashid, mirror_url=None):
...
def get_concrete_spec(args):
    ...
    if not spec_str and not spec_yaml_path:
        tty.msg("Must provide either spec string or path to " + "yaml to concretize spec")
        sys.exit(1)
...
def buildcache_copy(args):
    ...
    specfile_rel_path = os.path.join(build_cache_dir, bindist.tarball_name(spec, ".spec.yaml"))
