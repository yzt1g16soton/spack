...
def expected_patchelf_path(request, mutable_database, monkeypatch):
    ...
    which_fn = {
        "which_found": lambda x: collections.namedtuple("_", ["path"])("/usr/bin/patchelf")
    }
    monkeypatch.setattr(spack.util.executable, "which", which_fn.setdefault(case, lambda x: None))
    if case == "which_found":
