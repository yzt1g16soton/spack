...
def test_user_input_combination(config):
    ...
    possible_targets = [x for x in spack.architecture.platform().targets] + valid_keywords

    possible_os = [x for x in spack.architecture.platform().operating_sys] + valid_keywords
...
def test_concretize_target_ranges(root_target_range, dep_target_range, result):
    ...
    spec = Spec(
        "a %%gcc@10 foobar=bar target=%s ^b target=%s" % (root_target_range, dep_target_range)
    )
