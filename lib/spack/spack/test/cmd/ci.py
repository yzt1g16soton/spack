...
def test_ci_generate_with_env(
    tmpdir, mutable_mock_env_path, env_deactivate, install_mockery, mock_packages, project_dir_env
):
...
""".format(
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test") as env:
            ...
            buildcache_cmd(
                "download", "--spec-yaml", yaml_path, "--path", dl_dir.strpath, "--require-cdashid"
            )
...
def test_ci_subcommands_without_mirror(
    tmpdir, mutable_mock_env_path, env_deactivate, mock_packages, install_mockery, project_dir_env
):
