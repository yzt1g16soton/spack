...
class PyclingoDriver(object):
    def __init__(self, cores=True, asp=None):
        ...
        if not clingo:
            ...
            with spack.bootstrap.ensure_bootstrap_configuration():
                ...
                spack.bootstrap.make_module_available("clingo", spec=clingo_spec, install=True)
                import clingo
    ...
    def solve(
        self, solver_setup, specs, dump=None, nmodels=0, timers=False, stats=False, tests=False
    ):
        ...
        if result.satisfiable:
            ...
            criteria = [
                (int(args[0]), args[1]) for name, args in tuples if name == "opt_criterion"
            ]
    ...
    def external_packages(self):
        ...
        for pkg_name, data in packages_yaml.items():
            ...
            for version, weight, id in external_versions:
                self.gen.fact(fn.external_version_declared(pkg_name, str(version), weight, id))
