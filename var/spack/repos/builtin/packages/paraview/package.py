...
class Paraview(CMakePackage, CudaPackage):
    ...
    variant("plugins", default=True, description="Install include files for plugins support")
    ...
    def cmake_args(self):
        ...
        if spec.satisfies("+cuda") and not spec.satisfies("cuda_arch=native"):
            cmake_args.append("-DVTKm_CUDA_Architecture=%s" % spec.variants["cuda_arch"].value)
