...
class Timemory(CMakePackage):
    ...
    variant(
        ...
        default=False,
        description=(
            "Enable OpenMP tools support via drop-in " "replacement of libomp/libgomp/libiomp5"
        ),
    )
    variant(
        "ompt_llvm", default=False, description="Enable OpenMP tools support as part of llvm build"
    )
    ...
    conflicts("+python", when="~shared", msg="+python requires building shared libraries")
    ...
    conflicts("+kokkos_build_config", when="~tools", msg="+kokkos_build_config requires +tools")
    conflicts(
        "+kokkos_build_config",
        when="~kokkos_tools",
    ...
    conflicts(
        "+mpip_library", when="~shared", msg="+mpip_library requires building shared libraries"
    )
