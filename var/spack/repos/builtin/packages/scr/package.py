...
class Scr(CMakePackage):
    ...
    def cmake_args(self):
        ...
        args.append(
            "-DSCR_RESOURCE_MANAGER={0}".format(spec.variants["resource_manager"].value.upper())
        )

        if spec.satisfies("@:2.999"):
            args.append("-DSCR_ASYNC_API={0}".format(spec.variants["async_api"].value.upper()))

        args.append("-DSCR_FILE_LOCK={0}".format(spec.variants["file_lock"].value.upper()))
