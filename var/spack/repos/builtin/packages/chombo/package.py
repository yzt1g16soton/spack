...
class Chombo(MakefilePackage):
    ...
    def edit(self, spec, prefix):
        ...
        defs_file.filter(r"^\s*#\s*cppcallsfort\s*=\s*", "cppcallsfort = -DCH_FORT_UNDERSCORE")
        ...
        defs_file.filter(r"^\s*#\s*syslibflags\s*=\s*", "syslibflags = %s" % lapack_blas.ld_flags)

        # Compilers and Compiler flags
        defs_file.filter(r"^\s*#\s*CXX\s*=\s*", "CXX = %s" % spack_cxx)
        defs_file.filter(r"^\s*#\s*FC\s*=\s*", "FC = %s" % spack_fc)
        if "+mpi" in spec:
            defs_file.filter(r"^\s*#\s*MPICXX\s*=\s*", "MPICXX = %s" % self.spec["mpi"].mpicxx)

        # Conditionally determined settings
        defs_file.filter(
            r"^\s*#\s*MPI\s*=\s*", "MPI = %s" % ("TRUE" if "+mpi" in spec else "FALSE")
        )
        defs_file.filter(r"^\s*#\s*DIM\s*=\s*", "DIM = %s" % spec.variants["dims"].value)

        # HDF5 settings
        if "+hdf5" in spec:
            defs_file.filter(r"^\s*#\s*USE_HDF5\s*=\s*", "USE_HDF5 = TRUE")
            defs_file.filter(
                r"^\s*#\s*HDFINCFLAGS\s*=.*", "HDFINCFLAGS = -I%s" % spec["hdf5"].prefix.include
            )
            defs_file.filter(
                r"^\s*#\s*HDFLIBFLAGS\s*=.*", "HDFLIBFLAGS = %s" % spec["hdf5"].libs.ld_flags
            )
            if "+mpi" in spec:
                defs_file.filter(
