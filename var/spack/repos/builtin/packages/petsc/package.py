...
class Petsc(Package, CudaPackage, ROCmPackage):
    ...
    depends_on(
        "hypre@:2.13.99+mpi~internal-superlu~int64", when="@:3.8.99+hypre+mpi~complex~int64"
    )
    depends_on(
        "hypre@:2.13.99+mpi~internal-superlu+int64", when="@:3.8.99+hypre+mpi~complex+int64"
    )
    depends_on(
        "hypre@2.14:2.18.2+mpi~internal-superlu~int64", when="@3.9:3.13.99+hypre+mpi~complex~int64"
    )
    depends_on(
        "hypre@2.14:2.18.2+mpi~internal-superlu+int64", when="@3.9:3.13.99+hypre+mpi~complex+int64"
    )
    ...
    depends_on("hypre@2.14:+mpi~internal-superlu+int64", when="@3.14:+hypre+mpi~complex+int64")
    depends_on(
        "hypre@xsdk-0.2.0+mpi~internal-superlu+int64", when="@xsdk-0.2.0+hypre+mpi~complex+int64"
    )
    depends_on(
        "hypre@xsdk-0.2.0+mpi~internal-superlu~int64", when="@xsdk-0.2.0+hypre+mpi~complex~int64"
    )
    ...
    def install(self, spec, prefix):
        ...
        if spec.satisfies("+mumps"):
            ...
            options.extend(["--with-scalapack-lib=%s" % scalapack.joined(), "--with-scalapack=1"])
        else:
        ...
        if "+suite-sparse" in spec:
            ss_spec = (
                "suite-sparse:umfpack,klu,cholmod,btf,ccolamd,colamd," "camd,amd,suitesparseconfig"
            )
            options.extend(
                [
