...
class Xsdk(BundlePackage):
    ...
    depends_on("petsc@main+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64", when="@develop")
    depends_on("petsc@3.14.1+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64", when="@0.6.0")
    depends_on("petsc@3.12.1+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64", when="@0.5.0")
    depends_on("petsc@3.10.3+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64", when="@0.4.0")
    depends_on("petsc@3.8.2+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64", when="@0.3.0")
    depends_on(
        "petsc@xsdk-0.2.0+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64", when="@xsdk-0.2.0"
    )
    ...
    depends_on(
        "tasmanian@develop+xsdkflags+blas+cuda+magma" + tasmanian_openmp, when="@develop +cuda"
    )
    depends_on("tasmanian@7.3+xsdkflags+mpi+blas" + tasmanian_openmp, when="@0.6.0")
    depends_on(
        "tasmanian@7.3+xsdkflags+mpi+blas+cuda+magma" + tasmanian_openmp, when="@0.6.0 +cuda"
    )
    depends_on("tasmanian@7.0+xsdkflags+mpi+blas" + tasmanian_openmp, when="@0.5.0")
    depends_on(
        "tasmanian@7.0+xsdkflags+mpi+blas+cuda+magma" + tasmanian_openmp, when="@0.5.0 +cuda"
    )
